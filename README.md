...
An elimination **tournament** for **CompTIA CySA+ (CS0-004)** — Family Feud
...
`https://rafikiscyent888.github.io/Face-off-CySA/`.
...
Everything lives in **`questions-cysa.js`** — plain text, no code knowledge
...
questions**, weighted to the CS0-004 domains:

| Category | Domain (exam weight) |
|---|---|
| Architecture & Foundations | Security Operations (34%) |
| Indicators of Malicious Activity | Security Operations |
| Analysis Tools & Formats | Security Operations |
| Threat Intel, Hunting & AI | Security Operations |
| Scanning Approaches | Vulnerability Management (26%) |
| Analysis & Prioritization | Vulnerability Management |
| Treatment & Validation | Vulnerability Management |
| Attack Frameworks | Incident Response (24%) |
| Incident Response | Incident Response |
| Forensics & Evidence | Incident Response |
| Vulnerability Reporting | Reporting & Communication (16%) |
| Incident Communication | Reporting & Communication |

**CySA+ is an analyst exam**, so these questions lean on judgment rather than
recall — "you have a CVSS 9.8 on an isolated lab box and a 6.5 on an
internet-facing payment server, which do you fix first and why", not "what does
CVSS stand for". The 400s and 500s are all scenario calls.

**AI in security operations** is new in CS0-004 and is covered in the Threat
Intel, Hunting & AI category.

### About the objective tags

CompTIA has not published CS0-004's granular objective numbering (1.1, 1.2, …)
publicly yet, so clues are tagged by **domain and topic area** instead — which
are confirmed. If you have the official CS0-004 objectives PDF, drop the exact
numbers into the `obj` field of each clue and nothing else needs to change.
...
|---|---|---|
| **Normal** | The CompTIA CySA+ CS0-003 question bank, as always | Question bank |
...
**Don't edit `acronyms-cysa.js` by hand.** It's generated from the CySA-Acronyms repo,
which is the source of truth:

```
python3 tools/build-acronyms.py \
  ../cysa-acronyms/data/acronyms.json \
  acronyms-cysa.js FACEOFF_ACRONYMS "CYSA+" "CompTIA CySA+ CS0-003" cysa
```
...
they still play on shorter boards and in the Lightning Final.
...
| `acronyms-cysa.js` | The acronym bank — **generated**, see above |
| `tools/build-acronyms.py` | Rebuilds it from the CySA-Acronyms repo |
| `tools/acronym-topics.py` | Column headings for a bank that carries none |
| `questions-cysa.js` | **The question bank — this is the file you'll edit** |
